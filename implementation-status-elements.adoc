...
|nodeName ^|✓ ||
|baseURI ^|Not Planned ||Use base property insted
|ownerDocument ^|Not Planned ||Use getroottree() insted
|parentNode ^|Not Planned ||Use getparent() insted
|parentElement ^|Not Planned ||Use getparent() insted
|hasChildNodes() ^|Not Planned ||Use len(self) insted
|childNodes ^|Not Planned ||Use iterator insted
|firstChild ^|Not Planned ||
|lastChild ^|Not Planned ||
|previousSibling ^|Not Planned ||Use getprevious() insted
|nextSibling ^|Not Planned ||Use getnext() insted
|nodeValue ^|✓ ||
|textContent ^|✓ ||
...
|insertBefore() ^|Not Planned ||Use insert() insted
|appendChild() ^|Not Planned ||Use append() insted
|replaceChild() ^|Not Planned ||Use replace() insted
|removeChild() ^|Not Planned ||Use remove() insted
.19+|-|namespaceURI ^|✓ ||
|prefix ^|✓ ||
|localName ^|✓ ||
|tagName ^|✓ ||
...
|classList ^|TBD ||
|attributes ^|✓ ||
|getAttribute() ^|Not Planned ||Use attributes.get() insted
|getAttributeNS() ^|Not Planned ||Use attributes.get_ns() insted
|setAttribute() ^|Not Planned ||Use attributes.set() insted
|setAttributeNS() ^|Not Planned ||Use attributes.set_ns() insted
|removeAttribute() ^|Not Planned ||Use attributes.pop() insted
|removeAttributeNS() ^|Not Planned ||Use attributes.pop_ns() insted
|hasAttribute() ^|TBD ||
|hasAttributeNS() ^|TBD ||
...
|length ^|Not Planned ||Use len(data) insted
...
|DOMPoint |- | ^|Not Planned ||Use tuple class insted

|DOMRect |- | ^|Not Planned ||Use Rect class insted

|DOMQuad |- | ^|Not Planned ||

.56+|DOMMatrix <1> .22+|DOMMatrixReadOnly |fromMatrix() ^|Not Planned ||Use copy.deepcopy() insted
...
|toFloat64Array() ^|✓ ||Use toarray() insted
...
|valueInSpecifiedUnits ^|✓ ||Use value() insted
...
.6+|- |x ^|Not Planned ||Use get_computed_geometry() insted
|y ^|Not Planned ||Same as above
|width ^|Not Planned ||Same as above
|height ^|Not Planned ||Same as above
|instanceRoot ^|✓ ||
...
.12+|SVGTransformList .12+|- |length ^|✓ ||Use len(self) insted
|numberOfItems ^|✓ ||Use len(self) insted
|clear() ^|✓ ||
|initialize() ^|Not Planned ||
|getItem() ^|✓ ||Use s[i] insted
|insertItemBefore() ^|✓ ||Use insert() insted
|replaceItem() ^|✓ ||Use s[i] = t insted
|removeItem() ^|✓ ||Use remove() or pop() insted
|appendItem() ^|✓ ||Use append() insted
...
.3+|- |cx ^|Not Planned ||Use get_computed_geometry() insted
...
.4+|- |cx ^|Not Planned ||Use get_computed_geometry() insted
|cy ^|Not Planned ||Same as above
|rx ^|Not Planned ||Same as above
|ry ^|Not Planned ||Same as above

.5+|[[SVGLineElement]]SVGLineElement .1+|link:#SVGGeometryElement[SVGGeometryElement] |- |||
.4+|- |x1 ^|Not Planned ||Use get_computed_geometry() insted
...
|SVGPointList |- | ^|Not Planned ||Use list class insted
...
.5+|- |x ^|Not Planned ||Use get_computed_geometry() insted
|y ^|Not Planned ||Same as above
